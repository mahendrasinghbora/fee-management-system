...
if (isset($_SESSION['userid']) && ($_SESSION['userstatus'] == 'MANAGEMENT')) {
	...
	$navBrand = "View Defaulters";
	$result = $con->query("SELECT DISTINCT STUDENT_ID, CLASS_SESSION_ID FROM invoices;");
...
<html lang="en">
	<head>
		<title>FMS | View Defaulters</title>
		<!-- Required meta tags -->
		<meta charset="utf-8">
		...
		<div class="container-fluid mt-5">
			<div class="row px-5 mx-5">
				<table class="table table-bordered table-striped mt-3 bg-white table-sm">
					<caption class="lead">List of the students who have not cleared all of their dues (defaulters).</caption>
					<thead class="thead-dark">
						<tr>
							...
							<th scope="col">Remaining Dues (in <i class="fa fa-inr" aria-hidden="true"></i>)</th>
						</tr>
					</thead>
					<tbody>
						...
						<?php while ($row = $result->fetch_assoc()){
							...
							$resultDefaulter = $con->query("SELECT * FROM invoices WHERE STUDENT_ID = '$studentId' ORDER BY DATE_ DESC LIMIT 1;");
							if (mysqli_num_rows($resultDefaulter) != 0) {
								$rowDefaulter = $resultDefaulter->fetch_assoc();
								$previousDues = $rowDefaulter['PREVIOUS_DUES'];
								if ($previousDues != 0) { ?>
									<tr>
										...
										<td><?php echo $previousDues . " (" . ucfirst($inWords->format($previousDues)) . ".)"; ?></td>
									</tr>
								<?php } ?>
							<?php } ?>
						<?php } ?>
