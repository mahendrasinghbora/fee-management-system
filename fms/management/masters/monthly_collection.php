...
if (isset($_SESSION['userid']) && ($_SESSION['userstatus'] == 'MANAGEMENT')) {
	...
	$navBrand = "Monthly Fee Collection";
	$result = $con->query("SELECT SUM(ACTUAL_PAID_AMOUNT), substr(DATE_,6,2) FROM receipts GROUP BY substr(DATE_,6,2);");
?>

<!doctype html>
<html lang="en">
	<head>
		<title>FMS | Monthly Fee Collection</title>
		<!-- Required meta tags -->
		<meta charset="utf-8">
		...
		<div class="container-fluid mt-5">
			<div class="row px-5 mx-5">
				<table class="table table-bordered table-striped mt-3 bg-white table-sm">
					<caption class="lead">Monthly fee collection details.</caption>
					<thead class="thead-dark">
						<tr>
							<th scope="col">#</th>
							<th scope="col">Month</th>
							<th scope="col">Fee Collection (in <i class="fa fa-inr" aria-hidden="true"></i>)</th>
							<th scope="col">Fee Collection (in words)</th>
						</tr>
					</thead>
					<tbody>
						...
						<?php while ($row = $result->fetch_assoc()){ ?>
							<tr>
								...
								$month = $row['substr(DATE_,6,2)'] - 1;
								$inWords = new NumberFormatter("en", NumberFormatter::SPELLOUT);
								?>
								<td><?php echo $months[$month]; ?></td>
								<td><?php echo $row['SUM(ACTUAL_PAID_AMOUNT)']; ?></td>
								<td><?php echo ucfirst($inWords->format($row['SUM(ACTUAL_PAID_AMOUNT)'])) . "."; ?></td>
							</tr>
						<?php } ?>
					</tbody>
				</table>
